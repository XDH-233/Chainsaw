...
case class ConvInsideTile2D(config: ConvInsideTile2DConfig) { // FIXME
  import config._
  ...
  def randWeight(): Array[Array[Array[Array[Int]]]] = Array.fill(Tc)(Array.fill(Nic)(Array.fill(Krs)(Array.fill(Krs)(nextInt(10)))))
  ...
  def loopUnroll(ifMapTile: Array[Array[Int]], weightTile: Array[Array[Int]]): Array[Array[Int]] = { // FIXME
    ...
    for (to <- 0 until divideCeil(Tc, Uc); ti <- 0 until divideCeil(Nic, Uic)) {
  ...
  def weight2Tile(weight: Array[Array[Array[Array[Int]]]]): Array[Array[Int]] = { // oc -> ic -> r -> s // FIXME
    val ret = Array.fill(divideCeil(Nic, Uic) * divideCeil(Tc, Uc) * kernelSize * Uc)(Array.fill(Uic)(0))
    for (oc <- 0 until Tc; ic <- 0 until Nic; kr <- 0 until Krs; ks <- 0 until Krs) {
      ret((oc / Uc) * divideCeil(Nic, Uic) * kernelSize * Uc + (ic / Uic) * kernelSize * Uc + kr * Uc * Krs + ks * Uc + oc % Uc)(ic % Uic) =
        weight(oc)(ic)(kr)(ks)
    }
    ret
  }
  ...
  def ofMap2Tile(ofMap: Array[Array[Array[Array[Int]]]]): Array[Array[Int]] = {
    ...
    for (oc <- 0 until Tc; oh <- 0 until Nohw; ow <- 0 until Nohw; od <- 0 until Nd) {
  ...
  def loop(ifMap: Array[Array[Array[Array[Int]]]], weight: Array[Array[Array[Array[Int]]]]): Array[Array[Array[Array[Int]]]] = { // ifMap: ic -> ih -> iw -> id; weight: oc -> ic -> r -> s
    ...
    require(weight.length == Tc && weight.head.length == Nic && weight.head.head.length == Krs && weight.head.head.head.length == Krs, "weight size err")

    val ofMap = Array.fill(Tc)(Array.fill(Nohw)(Array.fill(Nohw)(Array.fill(Nd)(0)))) // oc -> oh -> ow -> od
