...
case class WeightBuffer(dataWidth: Int, uic: Int, depth: Int, readLatency: Int = 4) extends Component {
  val io = new Bundle {
  ...
  val urams: Seq[SPURAM] = Seq.fill(2)(SPURAM(width = dataWidth * uic, depth = depth, readLatency = readLatency, singlePortMode = ReadUnderWriteMode.NO_CHANGE))
