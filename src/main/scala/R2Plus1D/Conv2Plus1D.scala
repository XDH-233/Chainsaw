...
case class Conv2Plus1D(uic: Int = Parameter.Uic, uc: Int = Parameter.Uc, uoc: Int = Parameter.Uoc, dataWidth: Int = 8) extends Component {
  val io = new Bundle {
    val shortCut:              Bool = in Bool ()
    val weight2DRdy:           Bool = in Bool ()
    val weight1DRdy:           Bool = in Bool ()
    val fMapRdy:               Bool = in Bool ()
    val load2DConfig:          Bool = in Bool ()
    val load1DConfig:          Bool = in Bool ()
    val load1DResTo0DBuffer:   Bool = in Bool ()
    val elementWiseAdditionEn: Bool = in Bool ()
    val addition2To0DBBuffer:  Bool = in Bool ()
    ...
    val configPorts2D: ConfigParaPorts2D = in(ConfigParaPorts2D())
    val configPorts1D: ConfigParaPorts1D = in(ConfigParaPorts1D())
  }
  ...
  pingPongRegs2D.io.ifMapRdy        := io.fMapRdy
  pingPongRegs1D.io.weightRdy       := io.weight1DRdy
  loopCtrl2D.io.config.assignAllByName(io.configPorts2D)
  loopCtrl1D.io.config.assignAllByName(io.configPorts1D)
  loopCtrl1D.io.loadConfig := io.load1DConfig
  loopCtrl2D.io.loadConfig := io.load2DConfig
  ...
  // feature map read 1D
  featureMapBuffer.io.readEn1DPE   := io.shortCut & loopCtrl1D.io.ifMapRdEn
  ...
  outputBuffer2D.io.rdEn     := (~io.shortCut) & loopCtrl1D.io.ifMapRdEn
  ...
  pingPongRegs1D.io.loadConfig     := io.load1DConfig
  pingPongRegs1D.io.layerDone      := loopCtrl1D.io.layerDone
  pingPongRegs1D.io.ofMapSize2D    := io.configPorts1D.ofMapSizeOwOh
  ...
  pingPongRegs1D.io.ifMapRdy       := Mux(io.shortCut, True, loopCtrl2D.io.ofMapBuffer2DRdy)
  loopCtrl1D.io.readDone           := pingPongRegs1D.io.readDone

  // outputBuffer0D
  outputBuffer0D.io.we    := loopCtrl1D.io.ofMapWriteEn & io.load1DResTo0DBuffer
  outputBuffer0D.io.wAddr := loopCtrl1D.io.ofMapAddr
  outputBuffer0D.io.wData := accRAM1D.io.doutReLU

  outputBuffer0D.io.rdEn.clear()
  outputBuffer0D.io.rAddr.clearAll()
