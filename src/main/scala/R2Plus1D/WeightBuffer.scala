...
case class WeightBuffer(dataWidth: Int, uic: Int, depth: Int, pipeRegsCount: Int = 4) extends Component {
  ...
  val urams: Seq[SPURAM] =
    Seq.fill(2)(SPURAM(width = dataWidth * uic, depth = depth, pipeRegsCount = pipeRegsCount, singlePortMode = ReadUnderWriteMode.NO_CHANGE))
