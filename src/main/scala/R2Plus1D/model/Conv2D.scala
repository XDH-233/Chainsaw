...
case class Conv2D(config: ConvConfig) {
  ...
  def weight2Mem(weight: Array[Array[Array[Array[Int]]]]): Array[Array[Int]] = { // oc -> ic -> r -> s
    ...
    for (oc <- 0 until Nc; ic <- 0 until Nic; kr <- 0 until K; ks <- 0 until K) {
      ret((oc / Uc) * divideCeil(Nic, Uic) * kernelSize * Uc + (ic / Uic) * kernelSize * Uc + kr * Uc * K + ks * Uc + (Uc - 1 - oc % Uc))(
        ic % Uic
  ...
  private def weightAddr(to: Int, ti: Int, kr: Int, ks: Int): Int =
    to * divideCeil(Nic, Uic) * Uc * kernelSize + ti * Uc * kernelSize + kr * K * Uc + ks * Uc
